...
async function checkMemory() {
  ...
  */
    ...
    return { free: Used.free.gb, total: Used.total.gb, used: Used.used.gb, percent: Math.round(Used.percentUsed * 100) / 100 }; // retourne un objet contenant la mémoire libre, totale et le pourcentage d'utilisation de la mémoire
  } catch (err) {
    console.error(err);
  }
  return;
...
const { getDiskInfo, formatBytes } = require("node-system-stats"); // Importer les fonctions getDiskInfo et formatBytes pour obtenir les informations sur le disque
  ...
  /*
    ...
    return Math.round(Used[0].percentUsed * 100) / 100; // retourne le pourcentage d'utilisation du disque
